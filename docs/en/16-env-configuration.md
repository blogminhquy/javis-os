...
| `JAVIS_ENABLE_WEB_CHAT` | Force the `chatgpt-web` model (the engine driving a real ChatGPT browser session) on or off. `0`/`false`/`no`/`off` = force off | **Auto-detected** | Rarely needed. Javis detects it: with the `playwright` library present and a browser downloaded (the **Tools** page), the model appears in the model picker by itself; missing either one, it stays hidden and the ChatGPT card says exactly what is missing. Set `0` only when the machine has everything but you do NOT want anyone driving your ChatGPT session from Javis. Setting `1` cannot conjure the library or the browser, so it cannot force the feature on. |
