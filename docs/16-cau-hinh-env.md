...
| `JAVIS_ENABLE_WEB_CHAT` | Ép bật/tắt model `chatgpt-web` (engine chạy bằng phiên ChatGPT trong trình duyệt). `0`/`false`/`no`/`off` = ép tắt | **Tự dò** | Hầu như không cần đặt. Javis tự dò: có thư viện `playwright` và có trình duyệt (tải ở trang **Công cụ**) thì model tự hiện trong ô chọn model, thiếu một trong hai thì không hiện và thẻ ChatGPT nói rõ thiếu gì. Chỉ đặt `0` khi máy có đủ đồ nhưng bạn KHÔNG muốn ai lái phiên ChatGPT của mình từ Javis. Đặt `1` không tạo ra được thư viện hay trình duyệt, nên nó không ép bật được. |
