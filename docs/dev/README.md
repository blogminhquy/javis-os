...
- [Coding Workspace và Bàn giao phiên](2026-09-coding-workspace-spec.md) - mục thứ hai của nhóm Code (sau Terminal): một phiên coding gắn với một repo, một nhánh, một worktree và một mức quyền. Điểm cốt lõi: KHÔNG có auto switch model (chủ dự án chốt 2026-09-22: đổi engine giữa chừng làm hỏng mạch code), chỉ đổi bằng tay ở ranh giới an toàn; và gần như toàn bộ giao diện là node MƯỢN của trang khác (khung chat, panel Vault, `file-editor.js`, `model-picker.js`, `sessions-ui.js`), nên bề mặt mã mới chỉ còn sổ repo, hàng chip ngữ cảnh và điểm hồi git. Bỏ khỏi bản v0.1: quota router, cột Work Tree cố định, tầng Handoff riêng (đã có `conversation_state.py`), ép model API qua Codex CLI, và nhánh ASA. Bốn giai đoạn. **Chưa viết mã.**
