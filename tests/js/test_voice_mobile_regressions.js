...
assert.equal(shown, 'Em kiểm tra lại bản tin giá vàng.');
clearTimeout(v._silenceTimer);
assert.equal(Voice.ghepKetQua('không', 'không'), 'không không');
assert.equal(Voice.ghepKetQua('xem giá vàng', 'và giá bạc'), 'xem giá vàng và giá bạc');
assert.equal(Voice.ghepKetQua('Em kiểm tra.', 'em kiểm tra lại'), 'em kiểm tra lại');
v._iosCache = false;
v.recognition.start = () => {};
v.recognition.onend();
